...
vec4 AddFog(float distance,vec4 color,float depth)
{
	//float fog_factor = smoothstep( fogFactor.FOG_NEAR_PLANE, fogFactor.FOG_FAR_PLANE,depth );
	
    float fog_factor =   distance/500.f; 
    color = mix( color, vec4(1,1,1,1), fog_factor ); 
	return color;
}
